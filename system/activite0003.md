...
* grep 0 /etc/passwd
	...
	``` 
	jeanbourquj@lozan:~$ grep 0 /etc/passwd
	```
...
* cut -d: -f1,4 /etc/passwd 
	...
	```
	jeanbourquj@lozan:~$ cut -d: -f1,4 /etc/passwd
	```
...
* more /etc/group
	...
	``` 
	jeanbourquj@lozan:~$ more /etc/group
	```
...
* cut -d: -f1,3 /etc/group
	...
	<summary markdown="span">Commande 
	```
	jeanbourquj@lozan:~$ cut -d: -f1,3 /etc/group
	```
	</summary>
...
*	cat /etc/passwd | cut -d: -f1
	...
	``` 
	jeanbourquj@lozan:~$ cat /etc/passwd | cut -d: -f1
	```
...
* cat /etc/passwd | cut -d: -f1,6 | less
	...
	``` 
	jeanbourquj@lozan:~$ cat /etc/passwd | cut -d: -f1,6 | less
	```
...
* 	cat /etc/passwd | cut -d: -f1,6 | tee ./liste_comptes
	*	Affiche les collonnes '1' et '6' délimité par ':' du fichier /etc/passwd et créer un fichier 'liste_comptes' avec le contenu affiché
	...
	``` 
	jeanbourquj@lozan:~$ cat /etc/passwd | cut -d: -f1,6 | tee ./liste_comptes
	```
...
* sort < ./liste_comptes > ./liste_comptes.triee 
	...
	<details>
	<summary markdown="span">Commande de vérification 
	
	``` 
	jeanbourquj@lozan:~$ cat liste_comptes.triee
	```
	</summary>
...
* cat /etc/passwd | cut -d: -f1,6 | tee ./liste_comptes | less
	...
	``` 
	jeanbourquj@lozan:~$ cat /etc/passwd | cut -d: -f1,6 | tee ./liste_comptes | less
	```
	...
	```
	jeanbourquj@lozan:~$ cat liste_comptes
	```
...
*	indique le nom de l’utilisateur et l’IP de sa machine connectés au système sur le terminal 4 (/dev/pts/4)
	...
	```
	(effectué avec pts/0 car personne sur pts/4)
	jeanbourquj@lozan:~$ who | grep "pts/0" | tr -s [:space:] ':' | cut -d: -f1,6
	jeanbourquj:(10.7.0.1)
	```

>	aide : utiliser la commande tr (translate) : tr –s ' ' ':' file(s)

### 9)
_Analyser les commandes suivantes_
